 ...
 * anonymous guard, 90-char guard) and selfCheck.ts (/lab self-check). The
 * `phone_*` rows live in lines.phone.es.ts so the voter bundle stays small.
 *
...
import { PHONE_LINES } from "./lines.phone.es";
...
/** Projector-only pool (MascotHost). The phone pool lives in lines.phone.es.ts. */
export const PROJECTOR_LINES: Line[] = [
  // lobby_ambient
...
/** Full pool (projector + phone) for the /lab self-check and validators. */
export const LINES: Line[] = [...PROJECTOR_LINES, ...PHONE_LINES];
